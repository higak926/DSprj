...
public class RecipeRepository {
  ...
  public Optional<Recipe> selectById(long id) {
	...
	optionalRecipe.ifPresent(recipe -> {
	  List<RecipeDetail> recipeDetails = recipeDetailMapper.selectByExample(
  ...
  public void remove(final long id, final int version) {
	...
	int count = recipeMapper.deleteByExample(new RecipeEntityExample() {{
	  createCriteria().andIdEqualTo(id).andVersionEqualTo(version);
	}});

	if (count == 0) {
	  throw new OptimisticLockingFailureException(String.format("recipe id = [%d]", id));
	}
  }
